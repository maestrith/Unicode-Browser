...
Class Unicode_Browser{
	__New(Background:="Black",Color:="Pink"){
		...
		Loop,%Count%
		{
			...
			List.="<td Name='" A_Index+Start "' Title='&#x" Char "`n`n#x" Char "' ID='#x" Char ";'>" "&#x" Char ";" (CC?"#x" Char:"")
		}
		this.Body.InnerHTML:=List "</tr></table>Enter to go to next set, Shift+Enter to go to previous set, Click an item to Copy its Code, Right Click for Decimal<Style>table th,td{Border:1px Solid Grey;Padding:8px;Font-Size:50px}table td{Cursor:Hand}.ToolTipText{Visibility:Hidden}</Style>"
